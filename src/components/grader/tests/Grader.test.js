...
describe("Grader component", () => {
  ...
  it("should have the Temperatures tab", () => {
    ...
    expect(
      wrapper.find(".temperatures-tab > .mdc-tab__text-label").text()
    ).toEqual("Temperatures");
  });
  it("should go the Volumes tab", () => {
    const wrapper = shallow(<Grader />);
    expect(wrapper.find(".volumes-tab > .mdc-tab__text-label").text()).toEqual(
      "Volumes"
    );
  });
...
describe("Grader component integration tests", () => {
  ...
  it("should update step on form item change", () => {
    ...
    const inputField = container.querySelector("#inputValue");
